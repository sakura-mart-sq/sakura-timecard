...
4. 管理者が登録したメールアドレスと、6文字以上のパスワードを入力
   - 確認用パスワードも入力し、2つが一致している必要がある
   - 目のアイコンでパスワードの表示・非表示を切り替えられる
5. `Sign Up` を押す
...
Supabaseの再送制限が発生した場合も、内部の待機秒数は表示せず、送信済みメールを確認するよう通常色で案内します。受信済みのリンクはSupabaseで設定されたEmail OTP expirationまで有効です。認証失敗など実際のエラーだけを赤色で表示します。
