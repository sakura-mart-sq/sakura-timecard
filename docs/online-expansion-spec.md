...
スタッフ画面は英語です。入力フォームも英語です。上部には本人の名前とメールアドレスを表示し、ログアウトはハンバーガーメニューから行います。表示中は30秒ごと、およびブラウザへ戻ったときにデータを自動更新します。
...
管理者は計算結果をスタッフ単位で確定または公開できます。スタッフ画面に表示されるのは公開済みだけです。スタッフ本人による給与明細PDFの保存は別リリースとします。

#### 設定

- オンライン業務データのJSON完全バックアップ保存
- 旧localStorage版JSONバックアップの復元
- オンライン版JSONバックアップの復元
- スタッフ、スタッフコード、シフト、打刻、給与データの取り込み
- 店舗名、管理者パスコードの取り込み・保存
- シフト希望、交代申請がバックアップに存在する場合の取り込み
- 旧バックアップにそれらがなくても処理継続
- 復元前にログイン中管理者のSupabase Authパスワードを再確認
- パスワード確認後に、全置換の最終確認ダイアログを表示
- ログイン中の管理者本人によるSupabase Authパスワード変更

復元を実行した場合に限り、既存の業務データを削除してバックアップ内容へ置き換えます。
...
- シフト
- 打刻
- 給与データ（オンライン版バックアップにある場合）
- 店舗名
- 管理者パスコード
- シフト希望（バックアップにある場合）
- 交代申請（バックアップにある場合）

復元時は管理者のログインパスワードを再確認し、さらに最終確認を表示します。承認後、既存の業務データを削除してバックアップ内容へ置き換えます。Authユーザー、パスワード、管理者プロフィール、DB構造は削除しません。同じスタッフIDが存在するプロフィールの紐付けは復旧します。実行前に「完全バックアップ保存」で現在のデータを保存し、最初はテストDBで件数と給与を確認してから本番DBで実行します。
...
2026-09-17時点で、ローカル自動テスト28件と本番ビルドが成功しています。Supabaseの実データを使う認証、RLS、メール、Pages上の操作は、本番・テスト各プロジェクトのSQL適用状態と設定に依存します。
