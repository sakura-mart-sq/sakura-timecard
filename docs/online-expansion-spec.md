...
> このファイルは目標仕様と開発手順です。現在実際に使える機能・操作方法は [current-operation-guide.md](current-operation-guide.md) を正とします。特にメール通知、自動招待メール、旧localStorageとの自動同期は未実装です。
...
- 新しいスタッフのAuthユーザーは管理者のスタッフ登録処理からEdge Function経由で作成
...
現在の作業ブランチは `feature/shift-request-portal` です。以下のコミットをGitHubの店舗リポジトリへプッシュ済みで、Pull Requestを作成しています。
...
- 管理画面からスタッフAuthユーザーを招待・自動紐付け（Edge Function）
...
現在の作業ブランチ：feature/shift-request-portal

- [x] 管理者ログイン
- [x] スタッフ管理（追加・編集）
- [x] スタッフAuth招待・`profiles`自動紐付け
- [x] シフト希望の提出・取り下げ・管理者確認
- [x] シフト作成・編集
- [x] 勤務実績の確認・追加・修正
- [x] 週次給与計算の表示
