...
> 現在の操作方法と実装状況は、先に [current-operation-guide.md](current-operation-guide.md) を参照してください。このファイルには過去の開発経緯が残っています。

新しい店舗マネージャーの管理者アカウント追加手順とSQLは、同ガイドの「管理者アカウントの追加」に記載しています。

## 現在の状態

オンライン版の管理者ポータル、スタッフポータル、Supabase接続の店舗端末打刻まで実装済みです。旧localStorage版も互換性のため残っています。
...
- 自分の公開済み給与確認
- シフト希望の提出・取り下げ
- シフト交代申請、取り下げ、他スタッフの申請受諾
- スマートフォン・PCから打刻しない

### 店舗タブレット

- Supabase接続の端末モード（`?terminal=1`）
- 5桁コードによる打刻
- 旧localStorage版も維持
...
1. シフト交代申請のメール通知、メールからの受諾
2. 新規スタッフ登録時の自動招待メール
3. 登録端末の厳密な検証、オフライン対応
4. localStorageからSupabaseへの自動同期

## 次に実装する機能

### シフト交代申請

シフト交代の申請・画面上の受諾・先着1名の確定処理は実装済みです。現在はメール通知が未実装です。メール通知を追加する場合は、Edge FunctionではなくGASを使う方針で検討します。

スタッフAuth招待のEdge Functionは現在の画面から呼び出していません。スタッフ登録は、管理者がメールアドレスを保存し、スタッフ本人がログイン画面の`Sign Up`を使う方式です。シフト交代のRLSとRPC更新は`supabase/migrations/202609120001_shift_swap_access.sql`です。
...
管理者のオンライン画面には「バックアップから復元」があります。旧システムのJSONバックアップを選択すると、スタッフ、スタッフコード、シフト、打刻をSupabaseへ取り込みます。既存レコードは削除しないため、初回は空のテストDBで件数を確認してから本番DBで実行してください。
...
現在、自動テストは24件です。現在の利用方法は`docs/current-operation-guide.md`を更新し、機能追加時は実装状況も同時に更新します。
